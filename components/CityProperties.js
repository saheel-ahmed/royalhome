var CityProperties = {
    ...
    <div class="container-fluid" style="margin-top: 20px;">
         ...
         <div class="col-sm-8">
            <b>{{properties.length}}</b> properties are listed in <b>{{properties[0].Community}}</b>
         </div>
         <div class="col-sm-4 text-right">
                <a v-bind:href="'mailto:'+properties[0].Listing_Agent_Email" class="btn btn-sm btn-primary"><i class="houzez-icon icon-envelope mr-1"></i></a>
                <a v-bind:href="'tel:'+properties[0].Listing_Agent_Phone" class="btn btn-sm btn-light"><i class="houzez-icon icon-mobile-phone mr-1"></i></a>
         </div>
         </div>
  ...
  <button class="nav-link" @click="selectType('rent')" v-bind:class="{ active: selectedType == 'rent'}">RENT</button>
    ...
    methods:{
        fetchData: function () {
            let self = this;
            self.city = self.$route.params.name;
            $.getJSON('https://royalhome.ae/api/properties/', function (res) {
                self.properties = res.Listing.filter(i => i.Community.toLowerCase().replace(/ /gi, '-') == self.city);
